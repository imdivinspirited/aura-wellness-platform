...
import { getSession, getCurrentRootUserId, touchRootSessionActivity, clearSession } from '@/lib/root/auth';
import { publishRootOverrides } from '@/lib/api/root';
    ...
    (set, get) => ({
      ...
      activateRootMode: () => {
        ...
        if (local && Date.now() < local.expiresAt) {
          ...
          set({ isActive: true, session: local });
          return;
        }
        const auth = useAuthStore.getState();
        const roles = Array.isArray((auth.user as any)?.roles) ? ((auth.user as any).roles as string[]) : [];
        const isRoot = auth.user?.role === 'root' || roles.includes('root');
        if (!auth.isAuthenticated || !isRoot) {
          console.warn('[Root Store] Cannot activate root mode: user is not a root user');
          return;
        }
        set({ isActive: true, session: null });
      },
      ...
      publishChanges: async () => {
        const { pendingChanges, elementIndex } = get();
        const auth = useAuthStore.getState();
        const roles = Array.isArray((auth.user as any)?.roles) ? ((auth.user as any).roles as string[]) : [];
        const isRoot = auth.user?.role === 'root' || roles.includes('root');
        if (!auth.isAuthenticated || !isRoot) {
          console.error('[Root Store] Cannot publish: Not authenticated as root');
          return;
        }
        ...
        try {
          // Apply all pending changes to store (already applied for live preview, but keep consistent)
          for (const update of pendingChanges) {
            get().updateContent(update);
          }

          // Publish as DB-backed overrides (batch)
          const updates = pendingChanges.map((u) => {
            const selector = (u.metadata as any)?.selector || elementIndex[u.elementId]?.selector || `[data-root-id="${u.elementId}"]`;
            return {
              page: u.page,
          ...
          await publishRootOverrides(updates);
 ...
 */
async function persistContentMap(_contentMap: Record<string, ContentElement>): Promise<void> {
  // Legacy no-op: persistence is now DB-backed via publishRootOverrides()
}
