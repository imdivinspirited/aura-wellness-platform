...
/** Milliseconds until rate limit resets (0 if not limited or no record). */
export function getRateLimitRetryAfterMs(identifier: string): number {
  const record = rateLimitStore.get(identifier);
  if (!record) return 0;
  const now = Date.now();
  if (now > record.resetTime) return 0;
  if (record.count < RATE_LIMIT_CONFIG.maxQueriesPerMinute) return 0;
...
export function getRateLimitStatus(identifier: string): {
  ...
  return {
    limited,
    retryAfterMs: limited ? getRateLimitRetryAfterMs(identifier) : 0,
  };
}
