...
import { getSessionId, type DataSource } from '@/lib/chat/backendChatApi';
import { getAnswer, streamPlatformChat, type AnswerSource, type ChatMode } from '@/lib/chat/chatService';
...
export interface ChatMessage {
  ...
  /** Retrieved index rows (platform-chat meta frame) for transparency & accuracy. */
...
export function useChat(conversationId?: string | null) {
  ...
  const [mode] = useState<ChatMode>('platform');
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      if (mode === 'platform') {
        ...
        try {
          const history = buildConversationHistory([...messagesRef.current, userMsg]);
  ...
  return {
    ...
    loading,
    mode,
    setMode: (_m: ChatMode) => { /* platform-only, no-op */ },
    sendMessage,
    clearMessages,
    stopGeneration,
