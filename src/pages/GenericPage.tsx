...
import { translateNavDescription, translateNavLabel } from '@/lib/navI18n';
...
export const GenericPage = ({ path: pathProp, title }: GenericPageProps) => {
  ...
  const displayTitle =
    title ||
    (navItem ? translateNavLabel(navItem, tr) : null) ||
    t('common.notFound');
  const hasChildren = navItem?.children && navItem.children.length > 0;

  useSeoPageMeta({
    title: `${displayTitle} | The AOLIC Bangalore`,
  });
      ...
      <div className="container py-8 max-w-6xl">
          ...
          <Breadcrumb className="mb-6">
            <BreadcrumbList>
              ...
              {breadcrumbChain.map((item, index) => {
                ...
                const crumb = translateNavLabel(item, tr);
                return (
        ...
        {navItem && (
          <p className="text-muted-foreground text-lg">
            {translateNavDescription(navItem as NavItemWithDescription, tr) || t('empty.noData')}
          </p>
        )}
        </div>
                ...
                >
                  ...
                  <CardContent>
                    <p className="text-sm text-muted-foreground mb-4 line-clamp-2">
                      {translateNavDescription(childWithDescription, tr) || t('empty.noData')}
                    </p>
                    {child.href && (
...
import ServicesOverviewPage from './services/ServicesPage';
import { ExplorePage as ExplorePageComponent } from './explore/ExplorePage';
import { ConnectPage as ConnectPageComponent } from './connect/ConnectPage';

export const ProgramsPage = () => <ProgramsListingPage />;
export const ServicesPage = () => <ServicesOverviewPage />;
export const EventsPage = () => <EventsListingPage />;
export const ExplorePage = () => <ExplorePageComponent />;
export const OpportunitiesPage = () => <GenericPage path="/opportunities" title="Opportunities" />;
export const ConnectPage = () => <ConnectPageComponent />;
