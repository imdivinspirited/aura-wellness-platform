...
import { presignFormUpload, submitForm } from '@/lib/api/forms';
...
export function SevaApplicationForm({
  ...
  useEffect(() => {
    if (!open) return;
    // Best-effort prefill: pulls full profile (user + details) and fills matching fields.
    getMyProfile()
      .then((resp: any) => {
        const u = resp?.data?.user;
        const d = resp?.data?.details;
        if (u?.phone) form.setValue('phone', u.phone);
        if (d?.whatsapp) form.setValue('whatsapp', d.whatsapp);
        if (d?.age != null) form.setValue('age', d.age);
        ...
        if (d?.education) form.setValue('education', d.education);
        if (d?.skills) form.setValue('skills', d.skills);
        if (d?.availableFrom) form.setValue('availableFrom', String(d.availableFrom).slice(0, 10));
  ...
  const uploadFile = async (file: File, kind: 'resume' | 'photo'): Promise<{ url: string }> => {
    const presign = await presignFormUpload({
      kind,
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      let resumeUrl = '';
      if (resumeFile) {
        const uploaded = await uploadFile(resumeFile, 'resume');
      ...
      let photoUrl = '';
      if (photoFile) {
        const uploaded = await uploadFile(photoFile, 'photo');
      ...
      await submitForm({
        ...
        files: [
          ...(resumeUrl ? [{ kind: 'resume' as const, url: resumeUrl, originalName: resumeFile?.name, contentType: resumeFile?.type, sizeBytes: resumeFile?.size }] : []),
          ...(photoUrl ? [{ kind: 'photo' as const, url: photoUrl, originalName: photoFile?.name, contentType: photoFile?.type, sizeBytes: photoFile?.size }] : []),
        ],
      });
