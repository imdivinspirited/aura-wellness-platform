...
const EVENT_OVERRIDES_KEY = 'aolic-bangalore-events-overrides-v1';
...
export async function moveEventToPast(
  ...
  // Log admin action only when the admin flow is actually used.
  if (adminId) {
    try {
      const { logAdminAction } = await import('../admin/adminAuth');
      ...
      // Ignore audit logging import failures so the user action still succeeds.
    }
  }
 ...
 */
function mergeAllEventsWithOverrides(): Event[] {
  const overrides = loadOverrides();
...
/**
 * Client-only merged list (sync). Use for hero, detail, and list pages so localStorage overrides apply.
 */
export function getAllEventsSync(): Event[] {
  return mergeAllEventsWithOverrides();
}
