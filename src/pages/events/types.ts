...
export interface Event {
  ...
  // Dates & Time
  startDate: string; // ISO 8601 — focal / main programme day (cards, hero, countdown)
  endDate: string; // ISO 8601 — end of that focal window (can be same calendar day)
  timezone: string; // IANA timezone (e.g., 'Asia/Kolkata')
  isAllDay?: boolean; // For all-day events
  timeline?: EventTimeline[]; // Detailed schedule
  /**
   * When the “run” spans longer than `startDate`–`endDate` (e.g. Navratri 9 nights, or Gurudev
   * birthday month with main day on 13 May). Listing / ongoing–past logic uses this range.
   * If omitted, `startDate` and `endDate` define the whole window (hours-long or multi-day).
   */
  ongoingFrom?: string;
  /** Inclusive end of the ongoing period (e.g. last moment of May for a month-long arc). */
  ongoingUntil?: string;
  /**
   * Hours after `ongoingUntil` (or `endDate` when ongoing fields absent) before category becomes `past`.
   * Default 24. Short one-off programmes can set e.g. `6` for a tighter move to Archive.
   */
