...
export const EventCard = ({
  ...
  const effectiveCategory = getEffectiveCategory(event);
        ...
        <Card className="h-full flex flex-col border-stone-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="relative h-48 overflow-hidden rounded-t-lg bg-gradient-to-br from-primary/10 to-secondary/10">
            ...
            <div className="absolute top-4 left-4">
              <Badge
                variant={
                  effectiveCategory === 'upcoming'
                    ? 'default'
                    : effectiveCategory === 'ongoing'
                      ? 'destructive'
                      : 'secondary'
                }
              >
                {effectiveCategory === 'upcoming' && 'Upcoming'}
                {effectiveCategory === 'ongoing' && 'Ongoing'}
                {effectiveCategory === 'past' && 'Past'}
              </Badge>
            </div>

            {/* Days Until Badge (for upcoming events) */}
            {effectiveCategory === 'upcoming' && daysUntil > 0 && daysUntil <= 30 && (
          ...
          <CardFooter className="flex flex-col gap-2 pt-4 border-t border-stone-100">
            ...
            >
              <SmartLink to={event.cta.link} openInNewTab={Boolean(event.cta.openInNewTab)}>
                ...
                {event.cta.external && <ExternalLink className="ml-2 h-3 w-3" />}
              </SmartLink>
            </Button>

            {/* Admin Controls */}
            {showAdminControls && effectiveCategory !== 'past' && (
              <Button
                variant="outline"
                size="sm"
