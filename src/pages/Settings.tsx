...
import { Globe, Palette, Bell, Shield, Moon, Sun, Monitor, Lock, LogOut, Edit } from 'lucide-react';
import { useMemo } from 'react';
import { useRootStore } from '@/stores/rootStore';
import { useAuthStore } from '@/stores/authStore';
import { Link } from 'react-router-dom';
import { RootAccessButtons } from '@/components/root/RootAccessButtons';
import { useRootAuth } from '@/hooks/useRootAuth';
import { MarketingOpsLinksCard } from '@/components/settings/MarketingOpsLinksCard';
...
const Settings = () => {
  ...
  const { isActive, deactivateRootMode, activateRootMode } = useRootStore();
  const { user, isAuthenticated } = useAuthStore();
  const { isRootAuthenticated, logout: logoutRootSession } = useRootAuth();

  const isRootUser = useMemo(() => {
    if (!isAuthenticated) return false;
    const roles = Array.isArray((user as any)?.roles) ? ((user as any).roles as string[]) : [];
    return user?.role === 'root' || roles.includes('root');
  ...
  const canEnableRoot = isRootUser || isRootAuthenticated;
    ...
    <MainLayout>
      <div className="container py-10 max-w-4xl">
        ...
        <div className="space-y-6">
          ...
          <MarketingOpsLinksCard />

          {/* Root Access — sign in / sign up on dedicated pages; tools when allowed */}
          {!isActive ? (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Operator login for the root dashboard and visual editor. At most one root account exists per deployment.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <RootAccessButtons />
                {canEnableRoot && (
                  <div className="space-y-3 pt-2 border-t border-border/60">
                    {isRootAuthenticated && (
                      <Badge variant="outline" className="bg-emerald-700/10 text-emerald-800 border-emerald-700/30">
                        Root API session active
                      </Badge>
                    )}
                    <Button onClick={() => activateRootMode()} variant="outline" className="w-full">
                      <Edit className="mr-2 h-4 w-4" />
                      Enable Root Edit Mode
                    </Button>
                    <Button variant="secondary" className="w-full" asChild>
                      <Link to="/root/dashboard">Root dashboard</Link>
                    </Button>
                    <Button variant="secondary" className="w-full" asChild>
                      <Link to="/root/editor">Open visual editor</Link>
                    </Button>
                    {isRootAuthenticated && (
                      <Button
                        type="button"
                        variant="secondary"
                        className="w-full"
                        onClick={() => void logoutRootSession()}
                      >
                        Sign out root session
                      </Button>
                    )}
                  </div>
                )}
