...
import { samyamData } from './data';
import { motion } from 'framer-motion';
import { Card, CardContent } from '@/components/ui/card';

const SamyamPage = () => {
  const handleRegister = () => {
    window.open(samyamData.registrationUrl || 'https://programs.vvmvp.org/', '_blank', 'noopener,noreferrer');
  };

  return (
    <MainLayout>
      <ProgramHero data={samyamData} onRegister={handleRegister} />
      <ProgramBenefits benefits={samyamData.benefits} title="What Will I Get from This Program?" />

      {samyamData.whatIsProgram && (
        <section className="py-24 bg-white" aria-labelledby="what-is-program-heading">
          <div className="container max-w-4xl">
            ...
            >
              ...
              >
                {samyamData.whatIsProgram.title}
              </h2>
            </motion.div>
              ...
              <Card className="border-stone-200 shadow-sm">
                <CardContent className="p-8 md:p-12">
                  <div className="prose prose-stone max-w-none">
                    {samyamData.whatIsProgram.content.split('\n\n').map((paragraph, i) => (
              ...
              <Card className="border-stone-200 shadow-sm">
                <CardContent className="p-8 md:p-12">
                  <p className="text-stone-700 leading-relaxed mb-6">
                    {samyamData.whoIsItFor.content}
                  </p>
                  {samyamData.whoIsItFor.eligibility && (
                    <ul className="space-y-3">
                      {samyamData.whoIsItFor.eligibility.map((item, i) => (
          ...
          <div className="container max-w-6xl">
            ...
            >
              <h2
                ...
                className="font-display text-3xl md:text-4xl lg:text-5xl font-light mb-4 text-stone-900"
              >
                {samyamData.corePractices.title}
              </h2>
            </motion.div>

            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {samyamData.corePractices.practices.map((practice, i) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0, y: 30 }}
                  whileInView={{ opacity: 1, y: 0 }}
      ...
      <ProgramTestimonials testimonials={samyamData.testimonials} />
      <ProgramFounder founder={samyamData.founder} />
      <ProgramUpcoming programs={samyamData.upcomingPrograms} onRegister={handleRegister} />
      <ProgramFAQs faqs={samyamData.faqs} />
      <ProgramCTA
        onRegister={handleRegister}
        title="Ready for Advanced Practice?"
        subtitle="Experience the depth of Samyam - an advanced program for dedicated practitioners"
      />
    </MainLayout>
  );
};

export default SamyamPage;
