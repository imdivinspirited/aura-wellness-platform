...
import { panchkarmaData } from './data';
import { motion } from 'framer-motion';
import { Card, CardContent } from '@/components/ui/card';

const PanchkarmaPage = () => {
  const handleRegister = () => {
    window.open(panchkarmaData.registrationUrl || 'https://programs.vvmvp.org/', '_blank', 'noopener,noreferrer');
  };

  return (
    <MainLayout>
      <ProgramHero data={panchkarmaData} onRegister={handleRegister} />
      <ProgramBenefits benefits={panchkarmaData.benefits} title="What Will I Get from This Program?" />

      {panchkarmaData.whatIsProgram && (
        <section className="py-24 bg-white" aria-labelledby="what-is-program-heading">
          <div className="container max-w-4xl">
            ...
            >
              ...
              >
                {panchkarmaData.whatIsProgram.title}
              </h2>
            </motion.div>
              ...
              <Card className="border-stone-200 shadow-sm">
                <CardContent className="p-8 md:p-12">
                  <div className="prose prose-stone max-w-none">
                    {panchkarmaData.whatIsProgram.content.split('\n\n').map((paragraph, i) => (
          ...
          <div className="container max-w-4xl">
            ...
            >
              <h2
                ...
                className="font-display text-3xl md:text-4xl lg:text-5xl font-light mb-4 text-stone-900"
              >
                {panchkarmaData.whoIsItFor.title}
              </h2>
            </motion.div>
              ...
              <Card className="border-stone-200 shadow-sm">
                <CardContent className="p-8 md:p-12">
                  <p className="text-stone-700 leading-relaxed mb-6">
                    {panchkarmaData.whoIsItFor.content}
                  </p>
                  {panchkarmaData.whoIsItFor.eligibility && (
                    <ul className="space-y-3">
                      {panchkarmaData.whoIsItFor.eligibility.map((item, i) => (
          ...
          <div className="container max-w-6xl">
            ...
            >
              <h2
                ...
                className="font-display text-3xl md:text-4xl lg:text-5xl font-light mb-4 text-stone-900"
              >
                {panchkarmaData.corePractices.title}
              </h2>
            </motion.div>

            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {panchkarmaData.corePractices.practices.map((practice, i) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0, y: 30 }}
                  whileInView={{ opacity: 1, y: 0 }}
      ...
      <ProgramTestimonials testimonials={panchkarmaData.testimonials} />
      <ProgramFounder founder={panchkarmaData.founder} />
      <ProgramUpcoming programs={panchkarmaData.upcomingPrograms} onRegister={handleRegister} />
      <ProgramFAQs faqs={panchkarmaData.faqs} />
      <ProgramCTA
        onRegister={handleRegister}
        title="Ready for Deep Healing?"
        subtitle="Experience the transformative power of Panchkarma for complete detoxification and rejuvenation"
      />
    </MainLayout>
  );
};
