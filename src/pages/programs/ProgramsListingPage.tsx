...
import { getCanonicalUiLanguage, useTranslation } from '@/lib/i18n';
import { useUserStore } from '@/stores/userStore';

function catalogField(
  programId: string,
  field: 'title' | 'description' | 'level' | 'duration' | 'format',
  t: (key: string) => string,
  fallback: string
): string {
  const path = `programs.catalog.${programId}.${field}`;
  const v = t(path);
  if (v !== path) return v;
  return getCanonicalUiLanguage(useUserStore.getState().language) === 'en' ? fallback : path;
}
  ...
  const filteredPrograms = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((p) => {
        const title = catalogField(p.id, 'title', t, p.title);
        const description = catalogField(p.id, 'description', t, p.description);
        const level = catalogField(p.id, 'level', t, p.level);
        return (
      ...
      <section className="relative min-h-[40vh] flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          <img
            ...
            alt={t('programs.listing.heroAlt')}
        ...
        <div className="relative z-10 container py-16 text-center">
          <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-light mb-4 text-white">
            {t('programs.listing.heroTitle')}
          </h1>
          <p className="text-lg md:text-xl text-white/90 max-w-2xl mx-auto">
            {t('programs.listing.heroSubtitle')}
          </p>
        </div>
      </section>
        ...
        <div className="container">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-1 w-full max-w-md">
              ...
              <Input
                placeholder={t('programs.listing.searchPlaceholder')}
                value={searchQuery}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">
                {filteredPrograms.length === 1
                  ? t('programs.listing.programFound')
                  : t('programs.listing.programsFound').replace(
                      '{{count}}',
                      String(filteredPrograms.length)
                    )}
              </span>
            </div>
          </div>
          ...
          {filteredPrograms.length > 0 ? (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredPrograms.map((program, index) => {
                const title = catalogField(program.id, 'title', t, program.title);
                ...
                const level = catalogField(program.id, 'level', t, program.level);
                const duration = catalogField(program.id, 'duration', t, program.duration);
                const format = catalogField(program.id, 'format', t, program.format);
                return (
                  <motion.div
                    key={program.id}
            ...
            <div className="text-center py-20">
              <div className="max-w-md mx-auto">
                ...
                <h3 className="text-xl font-semibold mb-2">{t('programs.listing.emptyTitle')}</h3>
                <p className="text-muted-foreground mb-6">
                  {searchQuery
                    ? t('programs.listing.emptyHintSearch')
                    : t('programs.listing.emptyHintCategory')}
                </p>
                {searchQuery && (
                  <Button
                    variant="outline"
                  ...
                  >
                    {t('programs.listing.clearFilters')}
                  </Button>
                )}
              </div>
            </div>
