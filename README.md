# 🕉️ The AOLIC Bangalore — Art of Living Website
...
```bash
MONGODB_URI=mongodb://localhost:27017/the_aolic_bangalore
...
# SMTP (for Forgot/Reset password)
SMTP_HOST=
SMTP_PORT=587
SMTP_SECURE=false
SMTP_USER=
SMTP_PASS=
SMTP_FROM="The AOLIC Bangalore <no-reply@aolic-bangalore.local>"

# OAuth (social login) — use the same host/port as your API (see note below)
OAUTH_GOOGLE_CLIENT_ID=
OAUTH_GOOGLE_CLIENT_SECRET=
OAUTH_GOOGLE_CALLBACK_URL=http://localhost:4000/api/v1/oauth/google/callback
OAUTH_FACEBOOK_APP_ID=
OAUTH_FACEBOOK_APP_SECRET=
OAUTH_FACEBOOK_CALLBACK_URL=http://localhost:4000/api/v1/oauth/facebook/callback
OAUTH_GITHUB_CLIENT_ID=
OAUTH_GITHUB_CLIENT_SECRET=
OAUTH_GITHUB_CALLBACK_URL=http://localhost:4000/api/v1/oauth/github/callback
OAUTH_APPLE_CLIENT_ID=
OAUTH_APPLE_TEAM_ID=
OAUTH_APPLE_KEY_ID=
OAUTH_APPLE_PRIVATE_KEY=
OAUTH_APPLE_CALLBACK_URL=http://localhost:4000/api/v1/oauth/apple/callback
```

**Platform API (Express, `backend/`)** — copy `backend/.env.example` → `backend/.env`, run `npm run migrate`, then `npm run dev`. Default listen port is **4000** (`http://localhost:4000/api/v1`). The Docker `backend` service in `docker-compose.yml` still exposes **3000** if you use that stack instead.

**Frontend** (`.env` in project root — create from `.env.example`):

```bash
# Backend API base (must match the API you run: 4000 for `backend/` dev, 3000 for docker-compose backend)
...
### Social login (OAuth) callback URLs

- **Local platform API (`backend/` dev)**: `http://localhost:4000/api/v1`
- **Docker Compose `backend` service**: `http://localhost:3000/api/v1`
- **Local frontend**: `http://localhost:5173`

Set these callback URLs in the provider consoles (match the host/port you use):

- **Google**: `/api/v1/oauth/google/callback`
- **Facebook**: `/api/v1/oauth/facebook/callback`
- **GitHub**: `/api/v1/oauth/github/callback`
- **Apple**: `/api/v1/oauth/apple/callback`

After a successful social login, the backend redirects to the frontend callback page:
...
This repo includes production-ready Docker builds and a local/prod-parity `docker compose` stack.

**Local run**

1. Create `.env` in the repo root:
   - `JWT_SECRET=...`
   - `OPENAI_API_KEY=...` (required for chatbot embeddings + answers)
2. Run:
...
**Dev run (hot reload, one command)**
...
### Smoke testing (auth + core APIs)

After the backend is running, you can run an end-to-end smoke test (no extra dependencies):
...
- Anonymous session
- Register / login / refresh / `/auth/me`
- Mood APIs
- International overview
- Marketplace product listing
- Chatbot (best-effort; requires `OPENAI_API_KEY` + indexed content)

**Chatbot indexing (required for accurate answers)**

After you create/edit CMS content, rebuild the search index:

- `POST /api/v1/admin/search/reindex`

You must be logged in as a role with `content_admin` or `super_admin`.

**Seed + Reindex from UI**

In `/admin`:

- **Seed demo data**: creates real Products/Programs/Events/Services/Pages
- **Reindex chatbot**: rebuilds embeddings index from live DB content

### CI/CD (GitHub Actions)

Workflow: `.github/workflows/deploy.yml`

- **CI**: builds frontend + type-check/build backend on push to `main`
- **Deploy**: SSH to EC2 and runs `docker compose up -d`
...
They require `mongodump/mongorestore` and the AWS CLI on the machine where you run them.
...
*This README reflects the Art of Living Website master development prompt (v1.0) and **The AOLIC Bangalore** codebase.*
