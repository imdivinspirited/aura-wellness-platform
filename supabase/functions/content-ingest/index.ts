...
function buildEmbeddingText(item: Record<string, unknown>): string {
  ...
  if (raw) parts.push(raw.slice(0, 2800));
  ...
  if (typeof item.source_url === "string" && item.source_url) parts.push(`URL: ${item.source_url}`);
...
const STATIC_CONTENT = [
  ...
  },
  {
    ...
    content_type: "program",
    title: "Sri Sri Yoga",
    description: "Holistic yoga combining asanas, pranayama, and meditation. Suitable for all levels — beginners to advanced. Both online and in-person formats available.",
  ...
  },
  {
    ...
    description: "Effortless mantra-based meditation technique. 3 sessions over a weekend. Benefits: deep rest, clarity, reduced stress, better emotional balance.",
  ...
  },
  {
    ...
    description: "APEX (Achieving Personal Excellence) for professionals. Customized wellness programs for organizations. Focus: stress management, team building, leadership, productivity.",
  ...
  },
  {
    ...
    description: "Annual grand celebration at Bangalore Ashram (February/March). Live music, cultural performances, meditation. All-night celebration with special pujas. Free to attend.",
  ...
  },
  {
    ...
    description: "Art of Living International Center, 21st km, Kanakapura Road, Udayapura, Bangalore - 560082. Accommodation: Dormitories ₹300–₹800/night, Shared Rooms ₹800–₹1,500/night, Private Cottages ₹2,000–₹5,000/night. Pure vegetarian sattvic meals.",
  ...
  },
  {
    ...
    description: "65 acres of lush greenery. Vishalakshi Mantap (iconic lotus-shaped meditation hall), Sumeru Mantap, organic gardens, Sri Sri Ayurveda hospital, outdoor amphitheater, Sri Sri Tattva Shop, medical facilities.",
  ...
  },
  {
    ...
    description: "Volunteer positions available at the ashram. Kitchen Seva, Garden Seva, Media Seva, Admin roles, IT Support. Full-time job openings and internships (3-6 months).",
  ...
  },
  {
    ...
    description: "River rejuvenation projects, free education for 70,000+ underprivileged children, skill development in 40,000+ villages, disaster relief in 40+ countries, Prison SMART program in 600+ prisons.",
  ...
  },
  {
    ...
    description: "Founder of Art of Living Foundation (1981) and IAHV. Creator of Sudarshan Kriya. Humanitarian, spiritual leader, peace envoy. Present in 180+ countries, touched 500+ million lives.",
  ...
  },
  {
    ...
    description: "General: info@artofliving.org | +91-80-67262626. Programs: programs@artofliving.org. Corporate: corporate@artofliving.org. Ashram stay: ashram@artofliving.org.",
...
serve(async (req) => {
  ...
  try {
    ...
    const stats = { upserted: 0, deactivated: 0, wp_fetched: 0, embeddings_generated: 0, errors: [] as string[] };
      ...
      for (const ep of WP_ENDPOINTS) {
        try {
          ...
          const wpRes = await fetch(wpUrl, {
            headers: { "User-Agent": "AOL-ContentBot/2.0" },
          ...
          if (wpRes.ok) {
            ...
            if (Array.isArray(posts)) {
              for (const post of posts) {
                ...
                const upsertData: Record<string, unknown> = { ...item };
                if (embedding) {
    ...
    if (mode === "full" || mode === "static") {
      for (const item of STATIC_CONTENT) {
        const embeddingText = buildEmbeddingText(item as Record<string, unknown>);
        const embedding = await generateEmbedding(embeddingText);
    ...
    // Step 3: Generate embeddings for content that doesn't have them yet
    if (mode === "full" || mode === "embeddings") {
      const { data: noEmbedding } = await supabase
        .from("aol_content")
        .select("id, title, summary, description, category, venue, city, tags")
        .is("embedding", null)
        .eq("is_active", true)
        .limit(50);
      ...
      if (noEmbedding && noEmbedding.length > 0) {
        for (const row of noEmbedding) {
          const embText = buildEmbeddingText(row as Record<string, unknown>);
    ...
    // Step 4: Deactivate content with past end dates
    if (mode === "full") {
